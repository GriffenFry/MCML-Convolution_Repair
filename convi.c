 ...
 ****/
 /* This version is broken with modern compilers (12/13/2020) updated with FindDataLineTest
...
*/
char* FindDataLineTest(FILE* File_Ptr, char* Find_Buf)
{
	char buf[STRLEN];
	Find_Buf = buf;
	do {/* skip space or comment lines. */
		if (fgets(Find_Buf, STRLEN, File_Ptr) == NULL) {
			printf("Incomplete data.\n");
			exit(1);
		}
		CheckChar(Find_Buf);
	} while (CommentLine(Find_Buf));
	return (Find_Buf);
}
...
{
  char buf[STRLEN];
  char Find_Buf[STRLEN];

  do
    strcpy(buf, FindDataLineTest(File_Ptr,Find_Buf));
...
void ReadInParm(FILE* File_Ptr, InputStruct* In_Ptr)
{
	...
	do 
		strcpy(buf, FindDataLineTest(File_Ptr,Find_Buf)); // clear Find_Buf??
	while (!FoundKeyWord(buf, "InParm"));

	/** Escape filename & file format. */
	FindDataLineTest(File_Ptr,Find_Buf); // clear Find_Buf??
	 ...
	 OutStruct * Out_Ptr)
{
  ...
  for (i = 0; i < Na; i++) {
    /*
     * strcpy(buf, FindDataLineTest(File_Ptr,Find_Buf)); sscanf(buf, "%lf",
     * &(Out_Ptr->Tt_a[i]));
     */
